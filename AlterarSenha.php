...
$smtpPort = "587";
$sendmailFrom = "leandro.braga@odia.com.br";

// Configuração no PHP
ini_set("SMTP", $smtpServer);
ini_set("smtp_port", $smtpPort);
ini_set("sendmail_from", $sendmailFrom);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($user)) {
        ...
        if ($file = fopen($filename, 'w')) {
            ...
            $command = 'powershell.exe -File "D:\Html\AlterarSenha.ps1" 2>&1';
            exec($command, $output, $returnValue);

            $outputString = implode("\n", $output);
            // Adiciona essa linha para registrar a saída do PowerShell em um arquivo
            //file_put_contents('output_log.txt', $outputString); 

            // Adiciona essa linha para registrar o código de retorno em um arquivo
            //file_put_contents('return_value_log.txt', $returnValue); 
            ...
            if ($returnValue == 10) {
                ...
                $_SESSION['usuario'] = $user;
                $_SESSION['mensagem'] = "Usuário Desabilitado no AD.";
